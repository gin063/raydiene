...
onMounted(() => {
    ...
    // ============== 核心修复区：还原多行 Shader，强制全部使用 highp 高精度 ==============
  ...
  `;
    ...
    const blit = (() => {
        ...
        return (target: FBO | null, doClear = false) => {
            ...
            if (doClear) {
                gl.clearColor(0, 0, 0, config.TRANSPARENT ? 0.0 : 1.0);
    ...
    function initFramebuffers() {
        ...
        dye = !dye ? createDoubleFBO(dyeRes.width, dyeRes.height, rgba.internalFormat, rgba.format, texType, filtering) : resizeDoubleFBO(dye, dyeRes.width, dyeRes.height, rgba.internalFormat, rgba.format, texType, filtering);
        velocity = !velocity ? createDoubleFBO(simRes.width, simRes.height, rg.internalFormat, rg.format, texType, filtering) : resizeDoubleFBO(velocity, simRes.width, simRes.height, rg.internalFormat, rg.format, texType, filtering);
    ...
    function updateColors(dt: number) {
        ...
        if (colorUpdateTimer >= 1) {
            ...
            pointers.forEach((p) => {
                const hue = 0.48 + ((Math.sin(colorPhase * Math.PI * 2) + 1) / 2) * 0.12;
                p.color = generateColorFromHue(hue);
            });
        }
    }
    ...
    function clickSplat(pointer: Pointer) {
        const color = generateRandomColdColor();
    ...
    function updatePointerDownData(pointer: Pointer, id: number, posX: number, posY: number) {
        ...
        pointer.color = generateRandomColdColor();
    }
    ...
    function generateColorFromHue(hue: number): ColorRGB {
        const c = HSVtoRGB(hue, 1.0, 1.0);
        c.r *= 0.1; c.g *= 0.1; c.b *= 0.1;
        return c;
    }

    function generateRandomColdColor(): ColorRGB {
        const hue = 0.48 + Math.random() * 0.12;
        return generateColorFromHue(hue);
    }
    ...
    watch(() => props.simResolution, (newVal) => { config.SIM_RESOLUTION = newVal; initFramebuffers(); });
    watch(() => props.dyeResolution, (newVal) => { config.DYE_RESOLUTION = newVal; initFramebuffers(); });
