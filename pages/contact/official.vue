<template>
  <div
    ...
    <div class="fixed inset-0 z-0 pointer-events-none">
      <div
        class="absolute top-[-20%] left-[20%] w-[70vw] h-[70vw] bg-blue-900/10 rounded-full blur-[120px] opacity-30 animate-pulse-slow">
      </div>
      <div
        class="absolute bottom-[-10%] right-[-10%] w-[60vw] h-[60vw] bg-purple-900/10 rounded-full blur-[100px] opacity-20">
      </div>
    </div>
    ...
    <div class="relative z-10 container mx-auto px-6 md:px-12 pt-32 pb-16 md:pt-36 md:pb-16">
      ...
      <div class="mb-16">
        ...
        <div
          class="flex flex-nowrap overflow-x-auto snap-x snap-mandatory gap-4 pb-4 px-4 -mx-4 md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-6 md:overflow-visible md:pb-0 md:px-0 md:mx-0 no-scrollbar">

          <div
            class="flex-shrink-0 w-[85vw] snap-center md:w-auto group relative h-[420px] bg-white/5 border border-white/10 rounded-3xl overflow-hidden hover:border-cyan-500/50 transition-all duration-500 animate-fade-in-up"
            style="animation-delay: 0.2s;">

            <div
              class="absolute inset-0 flex flex-col items-center justify-center transition-opacity duration-300 group-hover:opacity-0 p-8">
              <div
                class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-[radial-gradient(closest-side,#0891b266_40%,transparent_100%)] pointer-events-none">
              </div>
            ...
            <div
              class="absolute bottom-10 left-0 w-full flex justify-center transition-opacity duration-300 group-hover:opacity-0 pointer-events-none">
            ...
            <div
              class="absolute inset-0 bg-cyan-600/20 backdrop-blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col items-center justify-center p-6 z-20">
              <div class="w-full flex justify-center gap-2 mb-6 bg-black/40 p-1 rounded-xl">
                <button @click.stop="jdTab = 'official'" @mouseenter="jdTab = 'official'"
                ...
                <button @click.stop="jdTab = 'self'" @mouseenter="jdTab = 'self'"
                  class="flex-1 py-2 text-sm font-bold rounded-lg transition-colors"
              ...
              <div class="relative w-40 h-40 bg-white p-2 rounded-xl">
                <Transition name="fade" mode="out-in">
                  <NuxtImg v-if="jdTab === 'official'" key="official" src="/images/qr/jd-official.jpg" alt="京东官方旗舰店"
                    class="w-full h-full object-contain" />
                  <NuxtImg v-else key="self" src="/images/qr/jd-self.jpg" alt="京东自营旗舰店"
                    class="w-full h-full object-contain" />
                </Transition>
              </div>
          ...
          <template v-for="(shop, index) in salesChannels" :key="shop.name">
            <div
              ...
              :style="{ animationDelay: (0.3 + index * 0.1) + 's' }" :class="shop.hoverBorder">
              <div
                class="absolute inset-0 flex flex-col items-center justify-center transition-all duration-300 group-hover:opacity-0 group-hover:scale-90 p-8">

                <div
                  class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] pointer-events-none"
                  :class="shop.glowClass"></div>

                <div
                  class="w-24 h-24 bg-white/10 rounded-full flex items-center justify-center mb-6 transition-colors relative z-10">
              ...
              <div
                ...
                :class="shop.hoverBg">
                <div
                  class="w-48 h-48 bg-white p-2 rounded-xl shadow-2xl transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                  <NuxtImg :src="shop.qr" :alt="shop.name" class="w-full h-full object-contain" />
                </div>
      ...
      <div>
        ...
        <div
          class="flex flex-nowrap overflow-x-auto snap-x snap-mandatory gap-4 pb-4 px-4 -mx-4 md:grid md:grid-cols-3 md:gap-6 md:overflow-visible md:pb-0 md:px-0 md:mx-0 no-scrollbar">
          <div v-for="(social, index) in socialChannels" :key="social.name"
            ...
            :style="{ animationDelay: (0.1 + index * 0.1) + 's' }" :class="social.hoverBorder">
            <div
              class="absolute inset-0 flex flex-col items-center justify-center transition-all duration-300 group-hover:opacity-0 group-hover:scale-90 p-8">

              <div
                class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] pointer-events-none"
                :class="social.glowClass"></div>

              <div
                class="w-20 h-20 bg-white/10 rounded-full flex items-center justify-center mb-6 transition-colors relative z-10">
            ...
            <div
              ...
              <div
                class="w-40 h-40 bg-white p-2 rounded-xl shadow-2xl transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                <NuxtImg :src="social.qr" :alt="social.name" class="w-full h-full object-contain" />
              </div>
              <div class="mt-6 text-center">
...
// --- SEO 配置 ---
useSeoMeta({
  title: '官方渠道 - 在线购买与社交媒体矩阵 | Raydiene',
  description: '访问雷迪恩(Raydiene)官方旗舰店选购正品充电桩。我们已入驻京东、天猫、拼多多及抖音商城。关注微信公众号与小红书，获取最新品牌资讯与服务。',
  keywords: '雷迪恩官方旗舰店, Raydiene京东, 天猫旗舰店, 拼多多, 抖音商城, 充电桩购买渠道, 雷迪恩微信公众号',
  // 社交分享优化
  ogTitle: '雷迪恩官方渠道导航',
